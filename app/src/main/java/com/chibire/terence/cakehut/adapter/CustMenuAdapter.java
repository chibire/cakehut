...
public class CustMenuAdapter  extends RecyclerView.Adapter<CustMenuAdapter.ViewHolder> {
    ...
    // 2
    public CustMenuAdapter(Context context) {
    ...
    public int getItemCount() {
        return new MenuData().menuList().size();
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        final restMenu rest = new MenuData().menuList().get(position);
        holder.placeName.setText(rest.name);
        Picasso.with(mContext)
                .load(rest.getImageResourceId(mContext))
                .error(R.drawable.logo)
                .into(holder.placeImage);

        Bitmap photo = BitmapFactory.decodeResource(mContext.getResources(), rest.getImageResourceId(mContext));
